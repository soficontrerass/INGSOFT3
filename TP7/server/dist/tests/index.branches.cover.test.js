...
test('non-production registers root handler that sends message and listens on default port 8080', async () => {
    await jest.isolateModulesAsync(async () => {
        ...
        const mockRes = { send: jest.fn(), sendFile: jest.fn() };
        rootHandler({}, mockRes);
        ...
        expect(logSpy).toHaveBeenCalledWith(`Server listening on port 8080 (FORECAST_COUNT=${process.env.FORECAST_COUNT || '5'})`);
        logSpy.mockRestore();
    });
});
// ...existing code...
